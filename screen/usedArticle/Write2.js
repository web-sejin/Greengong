...
import {ActivityIndicator, Alert, Button, Dimensions, View, Text, TextInput, TouchableOpacity, Modal, Pressable, StyleSheet, ScrollView, ToastAndroid, Keyboard, KeyboardAvoidingView, FlatList, BackHandler, findNodeHandle, NativeSyntheticEvent, TextInputFocusEventData} from 'react-native';
...
const Write2 = ({navigation, route}) => {
  ...
  const [size5, setSize5] = useState(''); //길이
	...
	const handleLayout13 = (event) => { const { x, y, width, height } = event.nativeEvent.layout; setLayout13({ x, y, width, height }); };
	...
	const getSaveState2 = async () => {
		...
		await Api.send('GET', 'get_temp_product', { 'is_api': 1, c1_idx: 2 }, (args) => {
			...
			if (responseJson.result === 'success' && responseJson) {
				...
				if(responseJson.pd_unit && responseJson.pd_unit != 0){ setPriceUnit(responseJson.pd_unit); }
				...
				if(responseJson.pd_weight){ setSize1(responseJson.pd_weight); }
        if(responseJson.pd_thickness){ setSize2(responseJson.pd_thickness); }
        if(responseJson.pd_outside){ setSize3(responseJson.pd_outside); }
        if(responseJson.pd_width){ setSize4(responseJson.pd_width); }
        if(responseJson.pd_length){ setSize5(responseJson.pd_length); }
  ...
  //   getFileCount(selectCon, 'del');
	...
	function writeUpdate(){
		...
		if (content == "") {
			...
			Keyboard.dismiss();
			scrollViewRef.current?.scrollToPosition(0, layout13.y, true);
			setState13(false);
			return false;
		}
		...
		<SafeAreaView style={styles.safeAreaView}>			
				...
				<View style={styles.registArea}>
					<View style={[styles.registBox]}>
						...
						<View style={[styles.typingBox, styles.mgTop35]} onLayout={handleLayout4}>
							...
							<View style={styles.typingAlert}>
								<AutoHeightImage width={17} source={require("../../assets/img/icon_alert3.png")} style={styles.typingAlertImg} />					
								<Text style={styles.typingAlertText}>성분을 입력해 주세요.</Text>
							</View>
							) : null}
						</View>
						) : null}
            ...
            <View style={[styles.typingBox, styles.mgTop35]}>							
                ...
                <TouchableOpacity
							...
							<View style={styles.typingAlert}>
								<AutoHeightImage width={17} source={require("../../assets/img/icon_alert3.png")} style={styles.typingAlertImg} />					
								<Text style={styles.typingAlertText}>가격 단위를 선택해 주세요.</Text>
							</View>
							) : null}
						</View>
						...
						<View style={[styles.typingBox, styles.mgTop35]} onLayout={handleLayout13}>
							<View style={styles.typingTitle}>
